...
void Accumulate(CImg<> ImgIn, CImg<> &Acc1, int x1, int x2, int y1, int y2){
        ...
        if(module(x1,y1) > 0){
            ...
            cimg_forXY(module,x2,y2)
            {
                ...
                if(abs((theta1 - theta2)) > 10){
                    //Center
                    //Accumulate(module, Acc1, x1,x2,y1,y2);
                    //Accumulate KN
...
*/
/*
void EllipseAccumulator(CImg<> ImgIn, CImg<> &Acc1)
{
...
*/
/*
void EllipseAccumulator(CImg<> ImgIn, CImg<> &Acc1, CImg<> &Acc2, CImg<> &Histo)
{
	...
	// Centres
	cimg_forXY(module, a0, b0)
	{
        
		if ( module(a0, b0) > DETECT ) {
            
			for (int ax = 0; ax < module.width(); ++ax) {
                
				for (int ay = 0; ay < module.height(); ++ay) {
                    
					for (int by = 0; by < module.width(); ++by) {
    ...
    /*cimg_forXY(module, x, y){
     if ( module(x, y) > DETECT ) {
     float gx = grad[0](x, y);
     float gy = grad[1](x,y);
     float phi1 = atan2(gy, gx);
     float phi2 = atan2(g2y, g2x);
     for(int K = 0; K  < 360; ++K){
     float p = atan(K);
     float N = sqrt(tan(phi1 - p) * tan(phi2 - p));
...
int main(int argc,char **argv)
{
    ...
    const char* filename = cimg_option("-i","ellipse.bmp","Input image file");
...
vector<int> Histo(img.width()/2);
    ...
    // Accumulator Computation
    EllipseAccumulator(img.get_channel(0), acc1);
